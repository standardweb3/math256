...
fn main()
{
  ...
  //Create a Uint256 with num256 crate
  let max128_uint256 = Uint256::from_bytes_le(&max128_bytes);

  //Add operator on Uint256 to get 
  let bignum = max128_uint256.clone() + Uint256::from_str("1024").unwrap();
  println!("This is the sum of 1024 and max uint 128 without overflow: {:?}",bignum); 

  //Just to check we can calculate the sub between big numbers 
  let subbignum = bignum - max128_uint256.clone();
  println!("Again we get back the addend of previous op: {:?}",subbignum);

  //If we did the same with u128 we panic! We must be careful because Rust in release mode won't panic.
  let _of128 = max128 + 2u128;
}
...
mod test {
    ...
    #[test]
    fn u256_zero(){
        let value = Uint256::min_value();
        assert_eq!(value, Uint256::from_str("0x00").unwrap());
    }

    #[test]
    fn u256_one(){
      let value = Uint256::min_value();
      let one = value + Uint256::from_str("1").unwrap();
      assert_eq!(one, Uint256::from_str("1").unwrap());
    }

    #[test]
    fn u256_maxplusone(){
      let max128 = u128::MAX.to_le_bytes();
      let maxplusone = Uint256::from_bytes_le(&max128) + Uint256::from_str("1").unwrap();
      assert_eq!(maxplusone, Uint256::from_bytes_le(&max128) + Uint256::from_str("1").unwrap());
    }
